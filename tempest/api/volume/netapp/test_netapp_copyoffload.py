...
TESTS WILL BOUNCE THE CINDER AND GLANCE SERVICES AND CHANGE CONFIG FILES.
...
class TestCopyOffload(base.BaseVolumeV2Test):
    def setUp(self):
        self.client = self.os.volumes_v2_client
        self.image_client = self.os.image_client_v2
    ...
    def _do_image_download_test(self):
        ...
        resp, volume = self.client.create_volume(1, display_name='vol-origin')
        ...
        resp, image = self.client.upload_volume(volume['id'], 'colImage',
                                                'raw')
        self.addCleanup(self._delete_image(image['id']))
        self.client.wait_for_volume_status(volume['id'], 'available')
        ...
        resp, new_vol = self.client.create_volume(1, display_name='vol-image',
                                                  imageRef=image['id'])
        volume = subprocess.check_output(["cinder",
                                          ...
                                          "1"])
        self.assertEqual(200, resp.status)
        self.assertIn('id', new_vol)
        self.addCleanup(self._delete_volume, new_vol['id'])
        # Wait for volume creation
        self.client.wait_for_volume_status(new_vol['id'], 'available')
